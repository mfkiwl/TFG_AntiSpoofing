...
%numberChannels=sum(size(trackResults.PRN,1));
numberChannels=size(channelList,2); %The 2 is so as to obtain the number of columns as channelList is a row vector
channelList=1:numberChannels;

%=== For all listed channels ==============================================
for channelNr = channelList
    switch trackResults(channelNr).status
        case 'APT' %Do nothing, the plots regarding the APT are the acquisition grids which are controlled by the initSettings_switch--> settings.AptPlots
        case 'T'
