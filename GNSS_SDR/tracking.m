...
for channelNr = 1:numberChannels% 1:settings.numberOfChannels
    ...
    %                     %receives as input 11ms of the FI-centered raw signal
                    ...
                    %order to detect if the spoofing attack is from a new
                    %satellite instead of the declared visibles in the
                    %first acquisition
...
% 
%                     acqType1='Normal';
%                     acqResults = acquisition(raw_signal_11ms, settings,acqType1,[]);
%                     plotAcquisition(acqResults);
        ...
        case 'T'
            ...
            if (channel(channelNr).PRN ~= 0)
                ...
                fseek(fid, ...
                      skipNumberOfBytes + channel(channelNr).codePhase-1, ...
                      'bof');
                ...
                for loopCnt =  1:codePeriods     
                    ...
                    % interation 
                    %[data] = readSignalFile(fid,settings,numSamples)
                    ...
                    switch settings.dataFormat
                        ...
                        case 'ishort'
                            % Finally compute the signal to mix the collected data to bandband
                            carrExp=exp(1j*trigarg(1:blksize));
                            %% Generate the six standard accumulated values ---------------------------
                            % First mix to baseband
                            baseband_samples_complex=rawSignal.*carrExp;
                            % Now get early, late, and prompt values for each
                            complex_E=sum(earlyCode  .* baseband_samples_complex);
                            complex_P=sum(promptCode  .* baseband_samples_complex);
                            complex_L=sum(lateCode  .* baseband_samples_complex);
                            I_E = real(complex_E);
                            Q_E = imag(complex_E);
                            I_P = real(complex_P);
                            Q_P = imag(complex_P);
